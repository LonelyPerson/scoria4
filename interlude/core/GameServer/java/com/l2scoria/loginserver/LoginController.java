...
//						_log.info("Closing "+cl.getIntetAddress()+" because idle time too long");
	...
	public void addLoginClient(L2LoginClient client)
	{
        ...
        public boolean PosibleMemoryOverflow() {
            long MemoryFree = (Runtime.getRuntime().maxMemory() - Runtime.getRuntime().totalMemory() + Runtime.getRuntime().freeMemory()) / 1048576;
            if(MemoryFree < Config.MIN_MEMORY_LEES) {
                return true;
            }
            return false;
        }
	...
	public boolean tryUseIP(InetAddress address) {
		Connection con = null;
		int id = 1;
		try {
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("SELECT id FROM accounts_activator WHERE ip=?");
	...
	public void removeActivatorIP(InetAddress address) {
		Connection con = null;
		try {
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("DELETE FROM accounts_activator WHERE ip=?");
			statement.setString(1, address.getHostAddress());
			statement.execute();
		} catch(Exception e) {
	...
	public boolean loginValid(String user, String password, L2LoginClient client)// throws HackingException
	{
		...
		try
		{
			...
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("SELECT password, access_level, lastServer, allowed_ip FROM accounts WHERE login=?");
			statement.setString(1, user);
			ResultSet rset = statement.executeQuery();

			if(rset.next())
			{
				String allowedIps = rset.getString("allowed_ip");
				if(allowedIps != null && !allowedIps.isEmpty() && !allowedIps.equals("*"))
				{
					...
					if(!allowedList.isIpInNets(address.getHostAddress()))
						return false;
				}
	...
	public void banControl(InetAddress address, String user, String password)
	{
		...
		{
			_hackProtection.put(address, new FailedLoginAttempt(user, password));
			failedCount = 1;
		}
		else
		{
			failedAttempt.increaseCounter(user, password);
	...
	class FailedLoginAttempt
	{
		...
		public FailedLoginAttempt(String user, String lastPassword)
		{
			_lastUser = user;
			_count = 1;
		...
		public void increaseCounter(String user, String password)
		{
			if(!_lastPassword.equals(password) || _lastUser != user)
			{
