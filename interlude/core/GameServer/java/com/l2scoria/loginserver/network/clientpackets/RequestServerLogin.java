...
import com.l2scoria.loginserver.network.serverpackets.PlayNotOk;
...
public class RequestServerLogin extends L2LoginClientPacket
{
	...
	public void run()
	{
		...
		if(Config.ANTI_BRUT_PROTECTION && getClient().getAccount()==null)
		{
			getClient().sendPacket(new PlayNotOk());
			return;
		}
		...
		if(!Config.SHOW_LICENCE || sk.checkLoginPair(_skey1, _skey2))
		{
			if(LoginController.getInstance().isLoginPossible(getClient(), _serverId))
			{
				...
				getClient().sendPacket(new PlayOk(sk, _serverId));
			}
			else
			{
