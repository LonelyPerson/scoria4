...
class OlympiadGameTask extends Olympiad implements Runnable
{
	...
	protected boolean proverkaDoTeleporta()
	{
		...
		try
		{
                    ...
                    {
                        if(_game._playerOne.gethwid() != null && _game._playerTwo.gethwid() != null)
                        {
                            if(_game._playerOne.gethwid().equals(_game._playerTwo.gethwid()))
                            {
                                _game._aborted = true;
                            }
                        }
		...
		if(_game._aborted)
		{
			if (!_pOneCrash)
			{
				_game._playerOne.sendMessage("Game aborted due to violation of registration rules.");
			}
			if (!_pTwoCrash)
			{
				_game._playerTwo.sendMessage("Game aborted due to violation of registration rules.");
			}
			_terminated = true;
	...
	protected boolean checkStatus()
	{
		...
		if(!Config.ALT_OLY_ALLOW_SAME_IP)
		{
			try
			{
				String ip_1 = _game._playerOne.getClient().getConnection().getInetAddress().getHostAddress();
				String ip_2 = _game._playerTwo.getClient().getConnection().getInetAddress().getHostAddress();
				if(ip_1.equalsIgnoreCase(ip_2) && ip_1 != null && ip_1 != "")
				{
					_game._playerOne.sendMessage("Game aborted due to violation of registration rules.");
					_game._playerTwo.sendMessage("Game aborted due to violation of registration rules.");
					_terminated = true;
		...
		if(_game != null)
		{
			if(_game._playerOne != null && _game._playerTwo != null)
			{
				...
				if(!_game.portPlayersToArena())
				{
					_terminated = true;
					_game._gamestarted = false;
					broadcastMessage(SystemMessage.sendString("Game terminated due to wrong conditions."), false);
					return;
				}
				_game.removals();
				try
				...
				if(STADIUMS[_game._stadiumID].getSpectators() != null)
				{
					for(L2PcInstance spec : STADIUMS[_game._stadiumID].getSpectators())
					{
						try
						{
	...
	private void broadcastMessage(SystemMessage sm, boolean toAll)
	{
		...
		if(toAll && STADIUMS[_game._stadiumID].getSpectators() != null)
		{
			for(L2PcInstance spec : STADIUMS[_game._stadiumID].getSpectators())
			{
				try
