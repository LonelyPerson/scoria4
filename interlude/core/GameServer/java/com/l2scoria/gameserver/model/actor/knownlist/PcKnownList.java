	...
	public boolean addKnownObject(L2Object object, L2Character dropper)
	{
		...
		else
		{
			...
			else if(object instanceof L2NpcInstance)
			{
				...
				{
					getActiveChar().sendPacket(new CustomNpcInfo((L2NpcInstance)object));
				}
				else
				{
			...
			else if(object instanceof L2PcInstance)
			{
				...
				else
				{
					...
					if (otherPlayer.getKnownList().getKnownRelations().get(getActiveChar().getObjectId()) == null)
						otherPlayer.getKnownList().getKnownRelations().put(getActiveChar().getObjectId(), -1);
					if(otherPlayer.getKnownList().getKnownRelations().get(getActiveChar().getObjectId()) != relation)
					{
						getActiveChar().sendPacket(new RelationChanged(otherPlayer, relation, getActiveChar().isAutoAttackable(otherPlayer)));
					}
				}
