...
public class L2Clan
{
	...
	public void setNewLeader(L2ClanMember member)
	{
		...
		if(Config.CLAN_LEADER_COLOR_ENABLED && getLevel() >= Config.CLAN_LEADER_COLOR_CLAN_LEVEL)
		{
			...
			{
				exLeader.getAppearance().setNameColor(0xFFFFFF);
			}
			else
			{
				exLeader.getAppearance().setTitleColor(0xFFFF77);
			}
		}
	...
	public L2PcInstance[] getOnlineMembers(int objectId)
	{
		...
		for(L2ClanMember temp : _members.values())
		{
			try
			{
				if(temp.isOnline() && temp.getObjectId() != objectId)
				{
	...
	{
		return objectId == 0 ? false : _members.containsKey(objectId);
	}
	...
	private void removeMemberInDatabase(L2ClanMember member, long clanJoinExpiryTime, long clanCreateExpiryTime)
	{
		...
		try
		{
			...
			{
				statement = con.prepareStatement("UPDATE characters SET clanid=0, title=?, clan_create_expiry_time=?, clan_privs=0, wantspeace=0, subpledge=0, lvl_joined_academy=0, apprentice=0, sponsor=0 WHERE obj_Id=?");
				statement.setString(1, "");
				statement.setLong(2, clanCreateExpiryTime);
	...
	public class SubPledge
	{
		...
		public L2ClanMember getLeader()
		{
			return getClanMember(_leaderId);
		}
	}
	...
	public int getLeaderSubPledge(int objectId)
	{
		int id = 0;
	...
	public boolean checkClanJoinCondition(L2PcInstance activeChar, L2PcInstance target, int pledgeType)
	{
		...
		int leadssubpledge = getLeaderSubPledge(activeChar.getObjectId());
