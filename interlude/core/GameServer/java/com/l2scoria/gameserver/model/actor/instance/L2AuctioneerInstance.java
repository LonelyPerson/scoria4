		...
		switch (validateCondition(player))
		{
			...
			case COND_REGULAR:
			{
					...
					try
					{
						...
						try
						{
							...
							if (bid < 1000000000)
								...
								player.sendMessage("The maximum bid is 999999999");
						}
						catch(Exception e)
						{
							player.sendMessage("Invalid bid!");
				...
				else if(actualCommand.equalsIgnoreCase("list"))
				{
					...
					StringBuilder items = new StringBuilder("<table width=280 border=0><tr>");

					for(int j = 1; j <= npage; j++)
					{
						items.append("<td><center><a action=\"bypass -h npc_").append(getObjectId()).append("_list ").append(j).append("\"> Page ").append(j).append(" </a></center></td>");
					}
					...
					for(Auction a : auctions)
					{
						...
						items.append("<tr><td width=50>").append(ClanHallManager.getInstance().getClanHallById(a.getItemId()).getLocation()).append("</td>").append("<td width=105><a action=\"bypass -h npc_").append(getObjectId()).append("_bidding ").append(a.getId()).append("\">").append(a.getItemName()).append("[").append(a.getBidders().size()).append("]").append("</a></td>").append("<td width=55>").append(format.format(a.getEndDate())).append("</td>").append("<td width=70>").append(a.getStartingBid()).append("</td>").append("</tr>");
					}
				...
				else if(actualCommand.equalsIgnoreCase("bidlist"))
				{
					...
					{
						biders.append("<tr>" + "<td>").append(b.getClanName()).append("</td><td>").append(b.getName()).append("</td><td>").append(b.getTimeBid().get(Calendar.YEAR)).append("/").append(b.getTimeBid().get(Calendar.MONTH) + 1).append("/").append(b.getTimeBid().get(Calendar.DATE)).append("</td><td>").append(b.getBid()).append("</td>").append("</tr>");
					}
