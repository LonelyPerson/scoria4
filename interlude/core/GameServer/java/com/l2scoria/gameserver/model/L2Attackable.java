...
public class L2Attackable extends L2NpcInstance
{
	...
	{
		// CommandChannel
		if(isRaid() && attacker != null && attacker.isInParty() && attacker.getParty().isInCommandChannel() && attacker.getParty().getCommandChannel().meetRaidWarCondition(this))
		{
				...
				synchronized (this)
				{
					if (_firstCommandChannelAttacked == null)
					{
						...
						if (_firstCommandChannelAttacked != null)
						{
							...
							ThreadPoolManager.getInstance().scheduleGeneral(_commandChannelTimer, 15000); // check for last attack
	...
	public void doItemDrop(L2NpcTemplate npcTemplate, L2Character lastAttacker)
	{
		...
		if(Config.L2JMOD_CHAMPION_ENABLE && isChampion() && (Math.abs(player.getLevel() - getLevel()) < Config.L2JMOD_CHAMPION_MAX_LVL_DIFF) && Config.L2JMOD_CHAMPION_REWARD > 0 && Rnd.get(100) < Config.L2JMOD_CHAMPION_REWARD)
		{
		...
		if(rateHp <= 1 && String.valueOf(npcTemplate.type).contentEquals("L2Monster")) //only L2Monster with <= 1x HP can drop herbs
		{
			...
			if(random < Config.RATE_DROP_SPECIAL_HERBS && !_spec) // && !_spec useless yet
			{
				...
				if(player.getAutoLoot() && Config.AUTO_LOOT_HERBS)
				{
