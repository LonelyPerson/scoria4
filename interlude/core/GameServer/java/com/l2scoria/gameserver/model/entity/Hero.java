...
public class Hero
{
	...
	private static final String UPDATE_HERO = "UPDATE heroes SET count = ?, played = ?, confirm = ? WHERE char_id = ?";
	...
	private void init()
	{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				hero.set(CONFIRM, rset.getInt(CONFIRM));
	...
	public int getConfirmed(int charId)
	{
		if(_heroes.size() != 0)
		{
			...
			if (hero != null)
			{
				return hero.getInteger(CONFIRM);
			}
		}
		return 0;
	}
			...
			else
			{
				...
				for(Integer heroId : _heroes.keySet())
				{
					...
					if(_completeHeroes == null || !_completeHeroes.containsKey(heroId))
					{
						...
						statement.setInt(6, hero.getInteger(CONFIRM));
						statement.execute();
					...
					else
					{
						...
						statement.setInt(3, hero.getInteger(CONFIRM));
	...
	public void confirmHero(int heroId)
	{
		if(_heroes.size() != 0)
		{
			StatsSet hero = _heroes.get(heroId);
