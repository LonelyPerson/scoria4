	...
	protected void onEnter(L2Character character)
	{
		if(character instanceof L2PcInstance)
		{
			...
			if(_enabled)
			{
				...
				{
					VanHalter.getInstance().intruderDetection(player);
					character.setInsideZone(L2Character.ZONE_NOSUMMONFRIEND, true);
					return;
				}

				if(Config.QA_TELEPORT_HIGHTERS && _zoneName.equalsIgnoreCase("LairofAntQueen") && player.getLevel() > 49 && !player.isGM())
				{
					if (Rnd.get(100) < 33)
						player.teleToLocation(-19480,187344,-5600);
					else if (Rnd.get(100) < 50)
						player.teleToLocation(-17928,180912,-5520);
					else
						player.teleToLocation(-23808,182368,-5600);
					return;
				}
			...
			// миша сказал выносить все в конфиги, нежели в зоны
			if(Config.ZAKEN_BANISH_CHEATERS && _zoneName.equalsIgnoreCase("LairofZaken"))
			{
				if(player.isGM() || DoorTable.getInstance().getDoor(21240006).getOpen())
				{
					character.setInsideZone(L2Character.ZONE_NOSUMMONFRIEND, true);
					return;
				}
