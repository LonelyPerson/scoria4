...
public final class L2RaidBossInstance extends L2MonsterInstance
{
	...
	{
		super.onSpawn();
		_returnHomeTask = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(new Runnable() {
			public void run()
			{
				// teleport raid boss home if it's too far from home location
				L2Spawn bossSpawn = getSpawn();
				if(!isInsideRadius(bossSpawn.getLocx(), bossSpawn.getLocy(), bossSpawn.getLocz(), 5000, true, false) && canReturnToSpawnPoint())
				{
					clearAggroList();
