...
public class TvTEvent
{
	...
	private static int highestLevelPcInstanceOf(final Map<Integer, L2PcInstance> players)
	{
		int maxLevel = Integer.MIN_VALUE, maxLevelId = -1;

		for(final L2PcInstance player : players.values())
	...
	public static boolean startFight()
	{
		...
		for(final L2PcInstance player : _teams[0].getParticipatedPlayers().values())
			if(player.isOnline() == 0)
			{
				removeParticipant(player.getObjectId());
			}
		for(final L2PcInstance player : _teams[1].getParticipatedPlayers().values())
			if(player.isOnline() == 0)
			{
				removeParticipant(player.getObjectId());
			}
	 ...
	 * 1. If both teams are at a tie(points equals), send it as system message to all participants, if one of the teams
	 * have 0 participants left online abort rewarding<br>
	...
	{
		final TvTEventTeam team = _teams[0];
		...
		for(final L2PcInstance playerInstance : team.getParticipatedPlayers().values())
		{
			...
			if(playerInstance.isOnline() == 0)
			{
				continue;
			}

			if(!Config.TVT_REWARD_WITH_NO_KILL && playerInstance.TvTKills == 0)
			{
				NpcHtmlMessage npcHtmlMessage = new NpcHtmlMessage(0);
				npcHtmlMessage.setHtml("<html><head><title>TvT Event</title></head><body>Your team won the event. But you don`t kill anyone. There is no reward for you.</body></html>");
			...
			for(final int[] reward : Config.TVT_EVENT_REWARDS)
			{
	...
	{
		final TvTEventTeam team = _teams[1];
	 ...
	 * @return boolean: true if success, otherwise false<br>
	 */
	public static synchronized boolean addParticipant(final L2PcInstance playerInstance)
	{
	 ...
	 * @return boolean: true if success, otherwise false<br>
	 */
	public static boolean removeParticipant(final int playerObjectId)
	{
	 ...
	 */
	public static void sysMsgToAllParticipants(final String message)
	{
		for(final L2PcInstance playerInstance : _teams[0].getParticipatedPlayers().values())
			if(playerInstance != null)
			{
				playerInstance.sendMessage(message);
			}

		for(final L2PcInstance playerInstance : _teams[1].getParticipatedPlayers().values())
			if(playerInstance != null)
			{
				playerInstance.sendMessage(message);
			}
	...
	{
		for(final int doorId : Config.TVT_DOORS_IDS_TO_OPEN)
		{
			L2DoorInstance door = DoorTable.getInstance().getDoor(doorId);
			try
			{
	 ...
	 * @param playerInstance as L2PcInstance<br>
	 */
	public static void onLogin(final L2PcInstance playerInstance)
	{
		...
		if(teamId == -1)
		{
			if(ArenaManager.getInstance().getArena(148695, 46725, -3414).isInsideZone(playerInstance))
			{
				playerInstance.teleToLocation(MapRegionTable.TeleportWhereType.Town);
			}
	 ...
	 * @param playerInstance as L2PcInstance<br>
	 */
	public static void onLogout(final L2PcInstance playerInstance)
	{
		if(playerInstance != null && (isStarting() || isStarted()))
		{
	 ...
	 * @param playerInstance as L2PcInstance<br>
	 */
	public static synchronized void onBypass(final String command, final L2PcInstance playerInstance)
	{
		...
		{
			NpcHtmlMessage npcHtmlMessage = new NpcHtmlMessage(0);
			final int playerLevel = playerInstance.getLevel();
			...
			else if(Olympiad.getInstance().isRegisteredInComp(playerInstance))
			{
				npcHtmlMessage.setHtml("<html><head><title>TvT Event</title></head><body>Players registered in olympiad are not allowed to participate.</body></html>");
			}
	 ...
	 */
	public static boolean onScrollUse(final int playerObjectId)
	{
		if(!isStarted())
			return true;
	 ...
	 */
	public static boolean onPotionUse(final int playerObjectId)
	{
		if(!isStarted())
			return true;
	 ...
	 */
	public static boolean onEscapeUse(final int playerObjectId)
	{
		if(!isStarted())
			return true;
	 ...
	 */
	public static boolean onItemSummon(final int playerObjectId)
	{
		if(!isStarted())
			return true;
	 ...
	 */
	public static void onKill(final L2Character killerCharacter, final L2PcInstance killedPlayerInstance)
	{
		...
		// change wait type
		killedPlayerInstance.broadcastPacket(new ChangeWaitType(killedPlayerInstance, ChangeWaitType.WT_START_FAKEDEATH));

		new TvTEventTeleporter(killedPlayerInstance, _teams[killedTeamId].getCoordinates(), false, false);

		killedPlayerInstance.broadcastPacket(new ChangeWaitType(killedPlayerInstance, ChangeWaitType.WT_STOP_FAKEDEATH));
		...
		if(killerTeamId != -1 && killedTeamId != -1 && killerTeamId != killedTeamId)
		{
			...
			for(final L2PcInstance playerInstance : _teams[killerTeamId].getParticipatedPlayers().values())
				if(playerInstance != null)
				{
	 ...
	 */
	private static void setState(final EventState state)
	{
		synchronized (_state)
		{
			_state = state;
	 ...
	 */
	public static byte getParticipantTeamId(final int playerObjectId)
	{
	 ...
	 */
	public static TvTEventTeam getParticipantTeam(final int playerObjectId)
	{
	 ...
	 */
	public static TvTEventTeam getParticipantEnemyTeam(final int playerObjectId)
	{
	 ...
	 */
	public static int[] getParticipantTeamCoordinates(final int playerObjectId)
	{
	 ...
	 */
	public static boolean isPlayerParticipant(final int playerObjectId)
	{
