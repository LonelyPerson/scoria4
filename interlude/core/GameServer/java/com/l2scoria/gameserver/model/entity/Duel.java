...
public class Duel
{
		...
		public void run()
		{
			try
			{
				...
				else if(status != DuelResultEnum.Continue)
				{
					setFinished(true);
					playKneelAnimation();
					if(Config.DUEL_WITH_ITEM_REQUIREMENT)
					{
						setPriseToWinner();
					}
	...
	public class ScheduleStartDuelTask implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				if(count == 4)
				{
					OlympiadStadia stadium = Olympiad.getInstance().getFreeStadium();
					if(stadium != null)
						_duel.teleportPlayers(stadium.getCoordinates()[0], stadium.getCoordinates()[1], stadium.getCoordinates()[2]);
					else
	...
	public void setPriseToWinner()
	{
		L2PcInstance winner = getWinner();
		winner.getInventory().addItem("DuelReward", Config.DUEL_CONSUM_ITEM_ID, Config.DUEL_CONSUM_ITEM_COST*2, winner, null);
		winner.sendMessage("You win in duel and obtain all items.");
	}
