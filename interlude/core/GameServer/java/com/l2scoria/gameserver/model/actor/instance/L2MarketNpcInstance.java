...
import com.l2scoria.gameserver.model.entity.market.Bid;
import com.l2scoria.gameserver.model.entity.market.Market;
import com.l2scoria.gameserver.model.entity.market.MarketTaxType;
 ...
 * @author zenn
 */
public class L2MarketNpcInstance extends L2FolkInstance {
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		...
		if(curCommand.startsWith("marketpage"))
		{
			...
			if(type.startsWith("index"))
			{
				...
				html.replace("%totalbids%", Market.getInstance().getBidsCount());
	...
	private String getMarketPage(int page)
	{
		...
		if(items.size() != 0)
		{
			...
			for(int i = page * Market.LOTS_PER_PAGE;i < numlines;i++)
			{
				...
				sb.append("<tr><td><font color=\"808080\">Тип: " + getItemType(items.get(i).getBidItem().getItem()) + ". Продавец: " + items.get(i).getBidder().getName() + "</font></td></tr>");
			}
			sb.append("</table>");
		...
		{
			sb.append("На рынке нету айтемов !");
		}
	...
	private NpcHtmlMessage genBidInfoPage(L2PcInstance player, int bidId)
	{
		...
		html.replace("%itemaugmentation%", bid.getBidItem().isAugmented() ? "Да" : "Нет");
		html.replace("%enchantlevel%", bid.getBidItem().getEnchantLevel());
		...
		if(bid.getTaxType() == MarketTaxType.BUYER)
		{
			...
			html.replace("%markettax%", (int)mtax + " <font color=\"LEVEL\">" + priceName + "</font> (Полная цена: " + (bid.getCostItemCount() + (int)mtax) + " <font color\"LEVEL\">" + priceName + "</font>)");
		}
		else
		{
			html.replace("%markettax%", "Платит продавец");
		}
		html.replace("%seller%", bid.getBidder().getName());
		if(player.getObjectId() == bid.getBidder().getObjectId())
		{
			html.replace("%LINKS%", "<a action=\"bypass -h npc_%objectId%_marketdel " + player.getObjectId() + " " + bid.getBidId() + "\">Убрать с рынка</a>");
		}
		else
		{
			html.replace("%LINKS%", "<a action=\"bypass -h npc_%objectId%_marketbuy " + bid.getBidId() + "\">Купить</a>");
		}
		return html;
	}
	...
	private NpcHtmlMessage genCabPage(L2PcInstance player)
	{
		...
		if(bids != null)
		{
			for(Bid bid: bids)
			{
				tx.append(bid.getBidItem().getItemName() + " +" + bid.getBidItem().getEnchantLevel() + "&nbsp;<a action=\"bypass -h npc_%objectId%_marketpage info " + bid.getBidId() + "\">Инфо</a>&nbsp;<a action=\"bypass -h npc_%objectId%_marketdel " + player.getObjectId() + " " + bid.getBidId() + "\">Убрать с рынка</a><br>");
			}
		}
		else
		{
			tx.append("У Вас нет айтемов на рынке.");
		}
	...
	private String BidItem(Bid bid)
	{
		...
		info += "<tr><td><font color=\"808080\">Цена: " + bid.getCostItemCount()  + " " + Market.getInstance().getShortItemName(bid.getCostItemId()) + "</font></td></tr>";
		return info;
	}
	...
	{
		html.replace("%backlink%", "<a action=\"bypass -h npc_%objectId%_marketpage index\">Назад</a>");
