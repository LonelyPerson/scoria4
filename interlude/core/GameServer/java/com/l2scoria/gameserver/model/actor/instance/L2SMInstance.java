...
public class L2SMInstance extends L2FolkInstance {
    ...
    /**
     *
     * @param player is a L2PcInstance character
     * @param premiumTime is a additional days
     */
    ...
    private void updateDatabasePremium(L2PcInstance player, boolean newDonator, long premiumTime, boolean depricated) {
        Connection con = null;
        try {
            if (player == null) {
                return;
            ...
            if (result.next()) {
                PreparedStatement stmt = con.prepareStatement(newDonator ? "UPDATE characters_custom_data SET donator=1, prem_end_date=? WHERE obj_Id=?" : "UPDATE characters_custom_data SET donator=0, prem_end_date=0 WHERE obj_Id=?");

                if (newDonator) {
                    stmt.setLong(1, premiumTime == 0L ? 0L : System.currentTimeMillis() + premiumTime);
                    stmt.setInt(2, player.getObjectId());
                    stmt.execute();
                } else {
                    stmt.setInt(1, player.getObjectId());
                ...
                stmt.setString(2, player.getName());
                stmt.setInt(3, 1);
                stmt.setLong(4, premiumTime == 0L ? 0L : System.currentTimeMillis() + premiumTime);
                stmt.execute();
                stmt.close();
                stmt = null;
            }
