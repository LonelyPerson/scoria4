...
public final class RequestDuelStart extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		else if(Config.DUEL_WITH_ITEM_REQUIREMENT)
		{
			L2ItemInstance item1 = activeChar.getInventory().getItemByItemId(Config.DUEL_CONSUM_ITEM_ID);
			if(item1 == null)
			{
				activeChar.sendMessage("You do not have required items to begin duel.");
				return;
			}
			if (item1.getCount() < Config.DUEL_CONSUM_ITEM_COST)
			{
				activeChar.sendMessage("You do not have enough items to begin duel.");
				return;
			}

			L2ItemInstance item2 = targetChar.getInventory().getItemByItemId(Config.DUEL_CONSUM_ITEM_ID);
			if(item2 == null)
			{
				activeChar.sendMessage("Your opponent does not have required items to begin duel.");
				return;
			}
			if (item2.getCount() < Config.DUEL_CONSUM_ITEM_COST)
			{
				activeChar.sendMessage("Your opponent does not have enough items to begin duel.");
				return;
			}

			if(!activeChar.destroyItemByItemId("Duel", Config.DUEL_CONSUM_ITEM_ID, Config.DUEL_CONSUM_ITEM_COST, null, false))
			{
				activeChar.sendMessage("You do not have enough items to begin duel.");
				return;
			}
			else if(!targetChar.destroyItemByItemId("Duel", Config.DUEL_CONSUM_ITEM_ID, Config.DUEL_CONSUM_ITEM_COST, null, false))
			{
				activeChar.sendMessage("Your opponent does not have required items to begin duel.");
				return;
			}
