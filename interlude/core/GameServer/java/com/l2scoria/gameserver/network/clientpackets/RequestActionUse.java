	...
	protected void runImpl()
	{
		...
		switch(_actionId)
		{
			...
			case 21: // pet follow/stop
				...
				{
					activeChar.sendMessage("Not allowed in siege mode.");
				}
				else if(pet != null && !pet.isMovementDisabled() && !activeChar.isBetrayed())
				{
			...
			case 37:
				...
				if(activeChar.getPrivateStoreType() == L2PcInstance.STORE_PRIVATE_MANUFACTURE || activeChar.getPrivateStoreType() == L2PcInstance.STORE_PRIVATE_MANUFACTURE + 1)
				{
				...
				if(activeChar.getPrivateStoreType() == L2PcInstance.STORE_PRIVATE_NONE)
				{
					...
					activeChar.setPrivateStoreType(L2PcInstance.STORE_PRIVATE_MANUFACTURE + 1);
				...
				if(pet != null && pet instanceof L2SiegeSummonInstance)
				{
					if(target != null && (target.isDoor || target instanceof L2SiegeFlagInstance))
					{
						if (((L2SiegeSummonInstance)pet).isOnSiegeMode())
							...
							activeChar.sendMessage("Can use only in siege mode.");
					}
					else
			...
			case 53: // move to target
				...
				else if(target != null && pet != null && pet != target && !pet.isMovementDisabled())
				{
