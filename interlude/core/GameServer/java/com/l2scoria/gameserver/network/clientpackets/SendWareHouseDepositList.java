...
public final class SendWareHouseDepositList extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		for(int i = 0; i < _count; i++)
		{
			...
			if(warehouse instanceof ClanWarehouse && item.isTimeLimitedItem())
			{
				player.sendMessage("Time-limited items can`t be deposited.");
				_items[i * 2 + 0] = 0;
				_items[i * 2 + 1] = 0;
				continue;
			}
		...
		// Check players on private store sell/buy
		// todo - add fail sendPacket() - i don`t know.
