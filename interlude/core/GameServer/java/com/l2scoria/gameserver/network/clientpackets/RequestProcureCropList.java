...
public class RequestProcureCropList extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		for(int i = 0; i < _size; i++)
		{
			...
			if(player.getInventory().getItemByObjectId(objId).getItemId() != cropId)
			{
				Util.handleIllegalPlayerAction(player, "Warning!! Character " + player.getName() + " of account " + player.getAccountName() + " tried to sell crops with invalid objectId", Config.DEFAULT_PUNISH);
				return;
			}
