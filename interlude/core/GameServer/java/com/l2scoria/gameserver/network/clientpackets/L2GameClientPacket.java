	...
	public void run()
	{
		try
		{
			...
			else
			{
				...
				if(this instanceof MoveBackwardToLocation || this instanceof AttackRequest || this instanceof Action)
				{
					//do nothing
				}
				else if(getClient().packetsSentInSec > Config.PROTECTED_ACTIVE_PACK_RETURN)
				{
					if(getClient().packetsSentInSec < Config.PROTECTED_ACTIVE_PACK_FAILED)
					{
