...
public final class RequestDropItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(!FloodProtector.getInstance().tryPerformAction(activeChar.getObjectId(), FloodProtector.PROTECTED_DROP))
		{
			if(Config.DEBUG)
			{
				_log.warn("Player " + activeChar.getName() + " has performed drop too fast");
			}
			activeChar.sendPacket(new SystemMessage(SystemMessageId.CANNOT_DISCARD_THIS_ITEM));
			return;
		}
		...
		if(_count <= 0)
		{
			Util.handleIllegalPlayerAction(activeChar, "[RequestDropItem] count <= 0! oid: " + _objectId + " owner: " + activeChar.getName(), IllegalPlayerAction.PUNISH_KICK);
			return;
		}
