...
public class Frintezza extends Quest
{
	...
	public Frintezza(int id, String name, String descr)
	{
		...
		for(int m : mob)
		{
			addEventId(m, Quest.QuestEventType.ON_KILL);
			addEventId(m, Quest.QuestEventType.ON_ATTACK);
		}
		addEventId(GUIDE, Quest.QuestEventType.QUEST_START);
		addEventId(GUIDE, Quest.QuestEventType.QUEST_TALK);
		addEventId(CUBE, Quest.QuestEventType.QUEST_START);
		addEventId(CUBE, Quest.QuestEventType.QUEST_TALK);
	...
	public String onAdvEvent (String event, L2NpcInstance npc, L2PcInstance player)
	{
			...
			if (GrandBossManager.getInstance().getBossStatus(FRINTEZZA) == FIGHTING)
			{
				if (npc != null)
				{
					...
					if (npc.getX() < 171932 || npc.getX() > 176532 || npc.getY() < -90320 || npc.getY() >-85720 || npc.getZ() < -5130)
						npc.teleToLocation(174232,-88020,-5116);
					startQuestTimer("loc_check", 60000, weakScarlet, null);
				}
			}
		}
		...
		{
			weakScarlet.updateAbnormalEffect();
		...
		else if (event.equalsIgnoreCase("morph_04"))
		{
			...
			if (skill != null)
			{
				weakScarlet.setTarget(weakScarlet);
		...
		else if (event.equalsIgnoreCase("morph_15"))
		{
			...
			if (skill != null)
			{
				strongScarlet.setTarget(strongScarlet);
		...
		else if (event.equalsIgnoreCase("songs_effect"))
		{
			...
			if (_OnSong == 1 || _OnSong == 2 || _OnSong == 3)
			{
				...
				{
					frintezza.setTarget(activeScarlet);
		...
		{
			_Zone.broadcastPacket(new CreatureSay(npc.getObjectId(),1,"Frintezza","Exceeded his time limit, challenge failed!"));
	...
	public String onKill (L2NpcInstance npc, L2PcInstance killer, boolean isPet)
	{
		...
		else if (npc.getNpcId() == SCARLET2 && _OnCheck == 1 && GrandBossManager.getInstance().getBossStatus(FRINTEZZA) == FIGHTING)
		{
			...
			long respawnTime = (long)Config.FRINTEZZA_RESP_FIRST + Rnd.get(Config.FRINTEZZA_RESP_SECOND);
