...
public class Nectar extends ItemAbst
{
	public Nectar()
	{
		_items = new int[]{6391};
	...
	public boolean useItem(L2PlayableInstance playable, L2ItemInstance item)
	{
		...
		if(!(player.getTarget() instanceof L2GourdInstance))
		{
			player.sendPacket(new SystemMessage(SystemMessageId.TARGET_IS_INCORRECT));
			return false;
		}

		if(player != ((L2GourdInstance) player.getTarget()).getOwner())
		{
			player.sendPacket(new SystemMessage(SystemMessageId.TARGET_IS_INCORRECT));
			return false;
		}

		player.useMagic(SkillTable.getInstance().getInfo(9999, 1), false, false);
		return true;
	}
}
