...
public class Potions extends ItemAbst
{
	...
	public Potions()
	{
		_items = new int[]{65, 725, 726, 727, 728, 734, 735, 1060, 1061, 1062, 1073, 1374, 1375, 1539, 1540, 5591, 5592, 6035, 6036, 6652, 6553, 6554, 6555, 8193, 8194, 8195, 8196, 8197, 8198, 8199, 8200, 8201, 8202, 8600, 8601, 8602, 8603, 8604, 8605, 8606, 8607, 8608, 8609, 8610, 8611, 8612, 8613, 8614, 8622, 8623, 8624, 8625, 8626, 8627, 8628, 8629, 8630, 8631, 8632, 8633, 8634, 8635, 8636, 8637, 8638, 8639, 8786, 8787};

		_requiresActingPlayer = true;
		_notOnOlympiad = true;
		_notInPvP = !Config.ALLOW_POTS_IN_PVP;
	}
	...
	public boolean useItem(L2PlayableInstance playable, L2ItemInstance item)
	{
		...
		switch (itemId)
		{
			...
			case 1073: // beginner's potion, xml: 2031
				...
				res = usePotion(activeChar, 2031, 1, itemId, 10000);
				break;
			case 1061: // healing_potion, xml: 2032
				...
				res = usePotion(activeChar, 2032, 1, itemId, 10000);
				break;
			...
			case 1539: // greater_healing_potion, xml: 2037
				...
				res = usePotion(activeChar, 2037, 1, itemId, 10000);
				break;
			...
			case 8627:
				...
				{
					res = usePotion(activeChar, 2287, (activeChar.getExpertiseIndex() + 1), itemId, 300000);
				}
				else
				{
			...
			case 8633:
				...
				{
					res = usePotion(activeChar, 2288, (activeChar.getExpertiseIndex() + 1), itemId, 300000);
				}
				else
				{
			...
			case 8639:
				...
				{
					res = usePotion(activeChar, 2289, (activeChar.getExpertiseIndex() + 1), itemId, 300000);
				}
				else
				{
					SystemMessage sm = new SystemMessage(SystemMessageId.INCOMPATIBLE_ITEM_GRADE); // INCOMPATIBLE_ITEM_GRADE
	...
	{
		return usePotion(activeChar, magicId, level, 0, 0);
	}

	public boolean usePotion(L2PcInstance activeChar, int magicId, int level, int itemId, int reusetime)
	{
		...
		else
		{
			...
			if (skill != null)
			{
				...
				{
					if (skill.getHitTime() > 10 || activeChar.isSomeSkillDisabled(magicId))
					{
						SystemMessage sm = new SystemMessage(SystemMessageId.S1_PREPARED_FOR_REUSE);
						sm.addSkillName(magicId, level);
						activeChar.sendPacket(sm);
						return false;
					}
				}
				...
				try
				{
					...
					if (itemId != 0 && reusetime != 0)
					{
						activeChar.sendPacket(new ExUseSharedGroupItem(itemId, -1, reusetime, reusetime));
					}
					activeChar.broadcastPacket(new MagicSkillUser(activeChar, activeChar, skill.getDisplayId(), level, skill.getHitTime(), reuseDelay));
					...
					if (reuseDelay > 30000)
					{
						activeChar.addTimeStamp(skill.getId(), reuseDelay);
					}
					if (reuseDelay > 10)
					{
					...
					if (handler != null)
					{
						handler.useSkill(activeChar, skill, skill.getTargetList(activeChar));
					}
					else
					{
						skill.useSkill(activeChar, skill.getTargetList(activeChar));
					}
				} catch (Exception e)
				{
					_log.warn("", e);
