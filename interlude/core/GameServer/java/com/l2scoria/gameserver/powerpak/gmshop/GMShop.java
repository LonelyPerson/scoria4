...
public class GMShop implements IVoicedCommandHandler, ICustomByPassHandler, IBBSHandler
{
	...
	private boolean checkAllowed(L2PcInstance activeChar)
	{
		...
		if (activeChar.isSitting())
			...
			msg = "GMShop не доступен когда вы кастуете заклинание";
		else if (activeChar.isDead())
			msg = "GMShop не доступен когда вы мертвы";
			...
			msg = "GMShop не доступен в данной зоне";
		else if(PowerPakConfig.GMSHOP_EXCLUDE_ON.contains("NOTINTOWN") && TownManager.getInstance().getTown(activeChar.getX(), activeChar.getY(), activeChar.getZ()) == null)
			msg = "GMShop не доступен в данной зоне";
