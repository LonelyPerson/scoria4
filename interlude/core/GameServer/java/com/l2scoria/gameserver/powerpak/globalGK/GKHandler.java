...
public class GKHandler implements IVoicedCommandHandler,ICustomByPassHandler, IBBSHandler
{
	...
	private boolean checkAllowed(L2PcInstance activeChar)
	{
		...
		if (activeChar.isSitting())
			...
			msg = "Гейткипер не доступен когда вы кастуете заклинание";
		else if (activeChar.isDead())
			msg = "Гейткипер не доступен когда вы мертвы";
			...
			msg = "Гейткипер не доступен в данной зоне";
		else if(PowerPakConfig.GLOBALGK_EXCLUDE_ON.contains("NOTINTOWN") && TownManager.getInstance().getTown(activeChar.getX(), activeChar.getY(), activeChar.getZ()) == null)
			msg = "Гейткипер не доступен в данной зоне";
