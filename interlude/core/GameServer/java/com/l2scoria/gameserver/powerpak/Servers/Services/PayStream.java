...
public class PayStream implements HttpHandler
{
	...
	private static String[] _DisableOperators;
	...
	public PayStream() throws Exception
	{
		...
		_DisableOperators = p.getProperty("DisableList", "").split(",");
			...
			if (query.get("skey")!=null && query.get("skey").equals(_KEY) && query.get("num")!=null) {
				...
				if(SMSText!=null) {
					...
					if(iPos !=-1) {
						...
						if(prefixOk) { // Проверяем на совпадение префикс (на всякий случай)
							try {
								Integer amount = _bonuses.get(Integer.parseInt(query.get("num")));
								if(amount==null)  // Указан ли наш номер в списке?
									amount = 0;   // Нет, то идите вы нахуй.
								for(String operator: _DisableOperators)
								{
									if(operator.equalsIgnoreCase(query.get("operator")))
									{
										amount = 0; // да пошел ты нахуй, бабки не даете.
									}
								}
								...
								if (amount > 0)
								{
									...
									response.append("Number is not registered.");
