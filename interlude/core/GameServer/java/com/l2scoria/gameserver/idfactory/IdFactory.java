...
public abstract class IdFactory
{
	...
	private void cleanUpDB()
	{
		...
		try
		{
			...
			//Character related
			cleanCount += stmt.executeUpdate("DELETE FROM buff_profile WHERE buff_profile.char_id NOT IN (SELECT obj_Id FROM characters);");
			...
			cleanCount += stmt.executeUpdate("DELETE FROM character_settings WHERE character_settings.charId NOT IN (SELECT obj_Id FROM characters);");
			...
			cleanCount += stmt.executeUpdate("DELETE FROM characters_custom_data WHERE characters_custom_data.obj_Id NOT IN (SELECT obj_Id FROM characters);");
			...
			cleanCount += stmt.executeUpdate("DELETE FROM rebirth_manager WHERE rebirth_manager.playerId NOT IN (SELECT obj_Id FROM characters);");
