...
{
	private static final Logger _log = Logger.getLogger(LoginTelnetThread.class.getName());
	...
	public LoginTelnetThread(Socket client, int uptime) throws IOException
	{
		_cSocket = client;
	...
	private boolean validLogin(String login)
	{
		...
		try
		{
			...
			if(rset.next())
			{
				...
				int alevel = 0;
				alevel = rset.getInt("access_level");
