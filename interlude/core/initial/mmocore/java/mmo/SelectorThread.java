...
import mmo.SelectorConfig.HeaderSize;
...
public class SelectorThread<T extends MMOClient> extends Thread
{
    ...
    protected void readPacket(SelectionKey key)
    {
		...
		// if the buffer is nearly full, try to close this connection.
		/*if (buf.position() >= buf.limit()-(int)buf.limit()/10)
        {
            // Some one try to attack server.
			con.onForcedDisconnection();
            closeConnectionImpl(con);
            System.out.println("Some one try to attack server... Connection close.");
			// Do we need to clear buf?? I`ll beter coment it.
			//buf.clear();
        }*/
        ...
        if (buf.position() == buf.limit())
        {
            ...
            //System.exit(0);
        	//buf.clear();
			System.out.println("Selector Thread: Try to read with no space in the buffer!"); 
			closeConnectionImpl(con);
			return;
        }
        ...
        {
            System.out.println("Made an attack on the server. Attacker: " + con.getSocketChannel().socket().getInetAddress().getHostAddress());
            closeConnectionImpl(con);
        }
        else if (result == -1)
        {
